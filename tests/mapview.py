...
def mapview(
    data, smooth=None, imrange="minmax", fig=None, ax=None, cmap="RdBu_r", wcs=None
):
    """
    Args
    ----
...
def fitsview(
    ...
    fig, ax = mapview(
        data, smooth=smooth, imrange=imrange, fig=fig, ax=ax, cmap=cmap, wcs=wcs
    )

    if isinstance(fwhm, u.Quantity):
        pix_size = (hdu.header["CDELT2"] * u.deg).to("arcsec")
        fwhm = (fwhm / pix_size).decompose().value
    if fwhm is not None:
        fwhm_plot = Ellipse(
            (fwhm, fwhm), fwhm, fwhm, edgecolor=beam_color, fill=False, hatch=r"xxxx"
        )
